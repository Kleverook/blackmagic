...
const command_s stm32f1_cmd_list[] = { { "option", stm32f1_cmd_option,
		"Manipulate option bytes" }, { NULL, NULL, NULL }, };



static bool stm32f1_flash_erase(target_flash_s *f, target_addr_t addr,
		size_t len);
static bool stm32f1_flash_write(target_flash_s *f, target_addr_t dest,
...
#define Power_control_register   (PWR_BASE + 0x00U)
...
#define FLASH_OBP_RDP_KEY1 0xAAU
...
static void stm32f1_add_flash(target_s *t, uint32_t addr, size_t length,
		size_t erasesize) {
...
if ((t->cpuid & CPUID_PARTNO_MASK) == CORTEX_M0
		|| (t->cpuid & CPUID_PARTNO_MASK) == CORTEX_M23)
...
case 0x438U: /* STM32F334C8 */
	t->driver = "STM32F334C8";
	t->part_id = device_id;
	target_add_ram(t, 0x20000000, 0x10000);
...
static inline void stm32f1_flash_clear_eop(target_s *const t,
...
static bool stm32f1_flash_busy_wait(target_s *const t,
	const uint32_t bank_offset, platform_timeout_s *const timeout) {
...
if ((t->part_id == 0x430U && end >= FLASH_BANK_SPLIT
...
static bool stm32f1_mass_erase_bank(target_s *const t,
	const uint32_t bank_offset, platform_timeout_s *const timeout) {
/* Unlock the bank */
...
target_mem_write32(t, 0xE000ED10, 0x4);
target_mem_write32(t, 0x40007000, 0x2);



//	target_mem_write32(t, FLASH_OBP_RDP, 0xffff0000U | FLASH_OBP_RDP_KEY1);
...
static bool stm32f1_option_write_erased(target_s *const t, const uint32_t addr,
	const uint16_t value, const bool write16_broken) {
if (value == 0xffffU)
	return true;
...
static bool stm32f1_option_write(target_s *const t, const uint32_t addr,
 ...
 */
const bool write16_broken = t->part_id
		== 0x410U&& (t->cpuid & CPUID_PARTNO_MASK) == CORTEX_M23;
for (size_t i = 0U; i < 8U; ++i) {
	if (!stm32f1_option_write_erased(t, FLASH_OBP_RDP + (i * 2U), opt_val[i],
			write16_broken) && i != 0)
		return false;
}
